...
NominalTypeDecl::getImplicitConversionInits() const {
  // Build the flat list lazily on first access, walking all members and
  // extensions. Heap-allocated because NominalTypeDecl is BumpPtrAllocated
  // and SmallVector has a non-trivial destructor.
  if (!ImplicitConversionInits) {
    ImplicitConversionInits = new llvm::SmallVector<ConstructorDecl *, 4>();
    auto consider = [&](Decl *member) {
      ...
      if (!params || params->size() != 1)
        return;
      ImplicitConversionInits->push_back(ctor);
    };
  ...
  return *ImplicitConversionInits;
}
